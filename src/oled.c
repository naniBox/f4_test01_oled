...
STATIC_ASSERT(LOGO_SIZE==sizeof(LOGO),               FRAME_BUFFER_LOGO_MISMATCH_3);
*/
...
void ssd1306_command(OLEDDriver * od, uint8_t c)
{
	OLED_write_command(od, c);
}
...
void OLED_put_pixel(OLEDDriver * od, uint8_t colour, int16_t x, int16_t y)
{
	...
	if (colour == 0)
		od->frame_buffer[x+ (y/8)*SSD1306_LCDWIDTH] &= ~(1 << (y&7));
	else
		od->frame_buffer[x+ (y/8)*SSD1306_LCDWIDTH] |= (1 << (y&7));
